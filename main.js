...
io.on("connection", (socket) => {
  ...
  if (!game.bannedIPs.includes(ipAddress)) {
    ...
    socket.on("requestPlayerData", (username, ackCallback) => {
      //create and return new player object from new client username
      if (!game.usedUsernames.includes(username.toLowerCase())) {
        const newPlayer = new Player(
          username,
          socket.id,
          !hasFirstPlayerJoined,
          ipAddress
        ...
        const responseData = {
          //data to be returned to player (formatted as object for possible future features)
  ...
  socket.on("update-self", (username, ackCallback) => {
    //returns corresponding player object to client
    const responseData = {
      //data to be returned to player
  ...
  socket.on("verifyAdminPassword", (input, ackCallback) => {
    //returns corresponding player object to client
  ...
  socket.on("submit-cards", (payload, ackCallback) => {
    //handles client card submission
    ...
    if (displaying) {
      ...
      for (let i = 0; i < cardSubmissions.length; i++) {
        if (cardSubmissions[i].text === text) {
          ...
          let payload = {
            msg:
              cardSubmissions[i].owner.name +
              " has won the round and their score is now " +
      ...
      game.players.forEach((player) => {
        ...
        if (game.playerLibrary[socket.id] != null) {
          if (player.name === game.playerLibrary[socket.id].name) {
            //if name matches DC-ing socket's associated name...
            playerIndex = game.players.indexOf(player); //get index of removed player
          }
        }
      });
      console.log(
        "A user has disconnected: " + game.playerLibrary[socket.id].name
      );
      game.players.splice(playerIndex, 1); //remove player from game list
      game.usedUsernames.splice(playerIndex, 1);
      if (game.playerLibrary[socket.id].czar && game.blackDeck) {
        //starts next turn if card czar leaves
...
class Deck {
  ...
  constructor(selectedPacks) {
    ...
    selectedPacks.forEach((pack) => {
      ...
      curPack["whiteCards"].forEach((white) => {
        //for white text component
        this.whiteDeck.push(new WhiteCard(white, pack)); //create new white card objects
      });
      curPack["blackCards"].forEach((black) => {
        //for each black card object
...
class Player {
  ...
  topUpCards() {
    while (this.hand.length < 10) {
      //draw white cards until hand length equals seven
...
class Game {
  constructor() {
    //game object (created when file is ran)
  ...
  addPlayer(player) {
    //adds and updates given player object
  ...
  setGamePhase(phase) {
    //runs code based on given game phase
    switch (phase) {
      case "submitting":
   ...
   */
  selectNextCzar() {
    //selects next card czar
    this.players.forEach((player) => {
      //remove czar status from all players
      player.czar = false;
    });

    if (!hasFirstTurnStarted) {
      //for first round, do not iterate to second player
      this.czarIndex = 0;
      this.czar = this.players[0];
    } else {
      //for all other rounds, iterate to next player in list
...
class Admin {
  //Static commands that can be run from the admin console
  static nukeGame() {
  ...
  static setPlayerScore(player, val) {
    //can be used to manually change a player's score
  ...
  static forceNextTurn() {
    //throws out current round and starts new round with next czar
...
function getRandom(min, max) {
  //shortcut function to make my life easier
...
server.listen(3000, () => {
  //listen for client connections and interactions @ port 3000
